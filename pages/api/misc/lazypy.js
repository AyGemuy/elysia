...
class Lazypy {
  ...
  async create({ service = "TikTok", voice = "es_male_m3", text = "Hy", voice_name = "Julio", playlist_index = 0 }) {
...
export default async function handler(req, res) {
  ...
  if (!type || !["create", "voice"].includes(type)) {
    return res.status(400).json({
      success: false,
      message: "Parameter 'type' harus diisi dengan 'create' atau 'voice'.",
    });
  }
  ...
  try {
    if (type === "voice") {
      ...
      if (!text) {
        return res.status(400).json({
          success: false,
          message: "Parameter 'text' wajib diisi untuk 'create'.",
        });
      }
      const ttsData = await lazypy.create({
